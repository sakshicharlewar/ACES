...
const LINE_BASE = { r: 255, g: 255, b: 255, a: 0.35 };
...
export default function KineticGrid({
  ...
  const draw = useCallback(
    (now: number) => {
      ...
      const theme = {
        default: {
          bg: "transparent", // Changed from #161618 to transparent so underlying HTML background can show
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      // Changed to use offsetWidth/offsetHeight of parent to only fill the section
    ...
    // FIX: Using window for mouse move makes sure it follows cursor anywhere in the window
    // as requested by the original implementation, but with adjusted coordinates.
    // However, to only react when hovering the section, we attach to window but calculate
    ...
    // Attach listeners to window so it responds well
