...
const BUILD_X_FALLBACK = {
  ...
  short_description:
    "BUILDX is a Full Stack Development Challenge where teams build innovative solutions while adapting to unexpected changes and evolving requirements. Participants must combine frontend, backend, databases, APIs, and real-time data to create a functional and scalable solution.",
    ...
    <section id="events" className="py-24 px-6 md:px-12 lg:px-24 relative overflow-hidden">
      <div className="container mx-auto">
        ...
        >
          ...
          <p 
            className="text-amber-100/80 text-lg md:text-xl font-serif italic mb-5"
            style={{ fontFamily: "'Cormorant Garamond', serif" }}
          >
            Participate &amp; Showcase your technical brilliance
          </p>

          {/* Join Official WhatsApp Group Pill */}
          <div className="flex items-center justify-center">
            <a
              href="https://chat.whatsapp.com/HgONFhA8qSbBr1zRhmWTir"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-[#25D366] hover:bg-[#20BD5A] text-black font-bold text-xs sm:text-sm transition-all duration-300 shadow-[0_0_25px_rgba(37,211,102,0.4)] hover:scale-105 active:scale-95 font-sans"
            >
        ...
        {isWakingUp && (
          <motion.div
            ...
            className="flex items-center justify-center gap-3 mb-8 px-5 py-3 rounded-2xl bg-white/5 border border-white/10 text-neutral-300 text-sm max-w-md mx-auto"
          >
          ...
          {displayEvents.map((event, index) => {
            ...
            return (
              ...
              >
                ...
                {isOpen && (
                  <div className="absolute inset-0 bg-gradient-to-br from-white/25 via-white/5 to-white/25 opacity-100 group-hover:animate-[spin_4s_linear_infinite] transition-all duration-500" />
                )}
                {!isOpen && (
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-[-100%] group-hover:animate-[marquee_2s_linear_infinite] opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                )}
                ...
                <div className="relative w-full h-full bg-[#111317]/85 backdrop-blur-xl rounded-[28px] p-6 flex flex-col border border-white/10 group-hover:border-white/20 group-hover:bg-[#15181E] transition-all duration-500 shadow-[0_12px_40px_rgba(0,0,0,0.4)]">
                  ...
                  {event.subtitle && <p className="font-sans text-xs font-semibold text-neutral-400 uppercase tracking-widest mb-3">{event.subtitle}</p>}
                  <p className="font-sans text-neutral-400 text-sm mb-4 line-clamp-3 leading-relaxed">{event.short_description || event.description}</p>

                  {/* Badges & meta */}
                  <div className="space-y-2 mb-6 text-sm text-neutral-300">
                    <div className="flex items-center justify-between mb-4">
                      {isResultAnnounced ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/20 border border-purple-500/40 text-purple-300 text-xs font-semibold uppercase tracking-wider shadow-[0_0_15px_rgba(168,85,247,0.3)]">
                          ✓ Completed
                        </span>
                      ) : isFull ? (
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/15 text-neutral-400 text-xs font-semibold uppercase tracking-wider">
                    ...
                    {(event.prizes || event.prize_pool) && (
                      <div className="flex items-center gap-2">
                        <span className="text-white">🏆</span>
                        <span className="text-neutral-300 text-sm truncate">{event.prizes || event.prize_pool}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-white/10">
                    ...
                    {isResultAnnounced ? (
                      <div
                        className="w-full py-3.5 rounded-full font-bold text-center text-sm bg-gradient-to-r from-purple-600/30 via-indigo-600/30 to-purple-600/30 text-purple-200 border border-purple-500/40 cursor-pointer hover:bg-purple-600/40 hover:shadow-[0_0_25px_rgba(168,85,247,0.45)] transition-all duration-300 flex items-center justify-center gap-2"
                        onClick={() => openWinnersPopup(event)}
                      >
                        🏆 View Winners &amp; Official Results
                      </div>
                    ) : isResultScheduled ? (
