...
export default function EventRegistrationModal({ isOpen, onClose, eventDetails, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (lastErr && (!response || response.status >= 400)) {
      ...
      setLoading(false);
      try {
        window.open(eventDetails?.google_form_url || "https://docs.google.com/forms/d/e/1FAIpQLSfPT8CxrR0GE85PZG0kguixqbYmC5mcXUa8BrAG6SPVXaosBw/viewform?usp=header", "_blank", "noopener,noreferrer");
      } catch (e) {}
      return;
    }
    ...
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-green-400 font-semibold text-sm mb-4">Your registration has been stored in the Admin Panel.</p>
          ...
          <div className="w-full bg-white/5 border border-white/10 rounded-xl p-4 mb-5 text-left space-y-2.5">
            ...
            <div className="flex items-start gap-2 text-xs text-neutral-300">
              ...
              <span>Your registration details &amp; team entries are saved in the Admin Database.</span>
            </div>
            <div className="flex items-start gap-2 text-xs text-neutral-300">
              <span className="text-amber-200 mt-0.5 shrink-0 font-bold">2.</span>
              <span>Join our official WhatsApp group &amp; fill out the Google Form confirmation:</span>
            </div>
            <div className="pt-2 flex flex-col gap-2.5">
              {(eventDetails?.whatsapp_link || pendingSuccessData?.whatsapp_link) && (
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer" 
                  className="inline-flex items-center justify-center gap-2 w-full py-3 px-4 bg-[#25D366] hover:bg-[#20BD5A] text-black font-bold rounded-full transition-all shadow-[0_0_20px_rgba(37,211,102,0.3)] text-sm font-sans"
                >
              ...
              <a 
                href={eventDetails?.google_form_url || "https://docs.google.com/forms/d/e/1FAIpQLSfPT8CxrR0GE85PZG0kguixqbYmC5mcXUa8BrAG6SPVXaosBw/viewform?usp=header"} 
                target="_blank" 
                rel="noreferrer" 
                className="inline-flex items-center justify-center gap-2 w-full py-3 px-4 bg-gradient-to-r from-amber-300 via-yellow-200 to-amber-400 hover:from-amber-400 hover:to-amber-500 text-black font-extrabold rounded-full transition-all shadow-[0_0_20px_rgba(251,191,36,0.35)] text-sm font-sans"
              >
