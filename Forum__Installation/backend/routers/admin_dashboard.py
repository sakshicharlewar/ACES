...
async def get_dashboard_stats(
    ...
    try:
        ...
        upcoming = (await db.execute(select(func.count(Event.id)).where(Event.event_status == getattr(EventStatus.upcoming, "value", "upcoming")))).scalar() or 0
        ongoing = (await db.execute(select(func.count(Event.id)).where(Event.event_status == getattr(EventStatus.ongoing, "value", "ongoing")))).scalar() or 0
        completed = (await db.execute(select(func.count(Event.id)).where(Event.event_status == getattr(EventStatus.completed, "value", "completed")))).scalar() or 0

        # Registration counts
        total_regs = (await db.execute(select(func.count(TeamRegistration.id)))).scalar() or 0
        approved = (await db.execute(select(func.count(TeamRegistration.id)).where(TeamRegistration.payment_status == getattr(PaymentStatus.approved, "value", "approved")))).scalar() or 0
        pending = (await db.execute(select(func.count(TeamRegistration.id)).where(TeamRegistration.payment_status == getattr(PaymentStatus.pending, "value", "pending")))).scalar() or 0
        rejected = (await db.execute(select(func.count(TeamRegistration.id)).where(TeamRegistration.payment_status == getattr(PaymentStatus.rejected, "value", "rejected")))).scalar() or 0
        ...
        for row in recent_regs_result.all():
            ...
            recent_regs.append({
                ...
                "payment_status": getattr(r.payment_status, "value", r.payment_status),
