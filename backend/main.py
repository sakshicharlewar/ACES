...
async def register_team(event_id: int, data: schemas.TeamRegistrationCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    # Generate robust registration ID with retry loop for race-condition safety
    try:
        max_retries = 5
        reg = None
...
async def admin_stats(request: Request, _=Depends(_verify_admin_token)):
    ...
    try:
        ...
        try:
            ...
            bh_remaining = max(0, (bh_event.max_teams if bh_event else 30) - bh_registered)
        except Exception as e:
...
async def startup_validation():
    ...
    try:
        with engine.begin() as conn:
            conn.execute(text("UPDATE upcoming_events SET max_teams = 30 WHERE title LIKE '%Bug Hunt%'"))
